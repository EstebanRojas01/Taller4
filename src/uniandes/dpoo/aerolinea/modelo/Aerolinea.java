...
public class Aerolinea
{
    ...
    {
    	int i=0;
    	while (i<vuelos.size()-1) {
    		i++;
    		String codBusca = vuelos.get(i).getFecha();
    		String dateBusca = vuelos.get(i).getRuta().getCodigoRuta();
    		if (codigoRuta == codBusca && fechaVuelo == dateBusca) {
    			return vuelos.get(i);}
    		}
    	return null;
    }
    ...
    {
		Vuelo vueloInicial = vuelos.get(0);
		Collection<Tiquete> tiquetes = vueloInicial.getTiquetes(); 
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	for(int i=0;i<=vuelos.size()-1;i++) {
    		Vuelo vuelo = vuelos.get(i);
    		String rutaCod = vuelo.getRuta().getCodigoRuta();
    		String nombre = vuelo.getAvion().getNombre();

        	if ( (nombre == nombreAvion) && (vuelo.getFecha()==fecha) && (rutaCod==codigoRuta)) {
                throw new Exception("El avion ya se encuentra ocupado en la fecha y ruta descrita");}}
    	Avion avionRetorno = null;
    	...
    	Ruta rutaRetorno = rutas.get(codigoRuta);
    	Vuelo vueloRetorno = new Vuelo(fecha,rutaRetorno,avionRetorno);
    	vuelos.add(vueloRetorno);
    } 
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
    	...
    	int calculo = cliente.calcularValorTotalTiquetes();
        return calculo +"";
    }
